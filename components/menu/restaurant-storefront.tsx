...
import { ArrowLeft, Leaf, MapPin, Minus, Plus, ShoppingBag } from "lucide-react";
...
export function RestaurantStorefront({ slug }: { slug: string }) {
  ...
  const itemCount = restaurant ? (cartState.cartsByRestaurant[restaurant.id]?.lines.reduce((total, line) => total + line.quantity, 0) ?? 0) : 0;
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        cartState.selectRestaurant(restaurantData.id);
        setActiveCategory(menuData[0]?.id ?? null);
      } catch (error) { if (live) setError(error instanceof Error ? error.message : "Menu could not be loaded."); }
  ...
  return <main className="min-h-screen bg-cream pb-24">
    <header className="sticky top-0 z-30 border-b border-orange-100 bg-cream/90 px-5 py-4 backdrop-blur"><div className="mx-auto flex max-w-6xl items-center justify-between gap-3"><Link href="/restaurants" className="inline-flex items-center gap-1 text-sm font-semibold text-stone-700"><ArrowLeft className="h-4 w-4" />All restaurants</Link><div className="flex items-center gap-2"><GuestProfileIndicator /><Button size="sm" onClick={() => cartState.setOpen(true)}><ShoppingBag className="h-4 w-4" />Cart {itemCount ? `(${itemCount})` : ""}</Button></div></div></header>
    <section className="mx-auto max-w-6xl px-5 pt-9"><div className="overflow-hidden rounded-[2rem] bg-gradient-to-br from-ink via-[#432418] to-saffron p-7 text-white sm:p-10"><p className="text-sm font-bold uppercase tracking-[.18em] text-orange-200">Independent kitchen</p><h1 className="display-font mt-3 text-4xl sm:text-6xl">{restaurant.name}</h1><p className="mt-4 max-w-2xl text-orange-50/80">{restaurant.description || "Freshly made favourites from a kitchen close to you."}</p><div className="mt-7 flex flex-wrap gap-3 text-sm"><span className="rounded-full bg-white/10 px-4 py-2"><MapPin className="mr-1 inline h-4 w-4" />{restaurant.delivery_radius_km} km delivery radius</span><span className="rounded-full bg-white/10 px-4 py-2">Base delivery from {formatINR(restaurant.delivery_fee_base)}</span></div></div></section>
    {categories.length === 0 ? <section className="mx-auto max-w-6xl px-5 py-14"><StatusNote>This restaurant is setting up its first menu. Please check back soon.</StatusNote></section> : <section className="mx-auto grid max-w-6xl gap-8 px-5 py-10 lg:grid-cols-[200px_1fr]"><aside className="lg:sticky lg:top-20 lg:h-fit"><p className="mb-3 text-xs font-bold uppercase tracking-[.16em] text-stone-500">Menu</p><nav className="flex gap-2 overflow-x-auto pb-2 lg:block lg:space-y-1">{categories.map((category) => <button key={category.id} onClick={() => document.getElementById(`category-${category.id}`)?.scrollIntoView({ behavior: "smooth", block: "start" })} className={`shrink-0 rounded-full px-4 py-2 text-sm font-semibold transition lg:block lg:w-full lg:text-left ${activeCategory === category.id ? "bg-ink text-white" : "bg-white text-stone-600 hover:bg-orange-100"}`}>{category.name}</button>)}</nav></aside>
      <div className="space-y-12">{categories.map((category) => <section id={`category-${category.id}`} key={category.id} ref={(element) => { refs.current[category.id] = element; }} className="scroll-mt-28"><div className="mb-5"><h2 className="display-font text-3xl">{category.name}</h2>{category.description && <p className="mt-1 text-sm text-stone-600">{category.description}</p>}</div><div className="grid gap-4 sm:grid-cols-2">{category.items.map((item) => { const quantity = cartState.cartsByRestaurant[restaurant.id]?.lines.find((line) => line.itemId === item.id)?.quantity ?? 0; return <Card className="p-5" key={item.id}><div className="flex items-start justify-between gap-4"><div><div className="flex items-center gap-2"><h3 className="font-bold text-ink">{item.name}</h3>{item.vegetarian && <Leaf className="h-4 w-4 text-moss" aria-label="Vegetarian" />}</div><p className="mt-2 min-h-10 text-sm leading-5 text-stone-600">{item.description || "Prepared fresh when you order."}</p></div><span className="whitespace-nowrap font-bold">{formatINR(item.price)}</span></div><div className="mt-5 flex justify-end">{quantity ? <div className="flex items-center gap-3 rounded-full bg-orange-100 p-1"><button aria-label={`Remove one ${item.name}`} className="grid h-8 w-8 place-items-center rounded-full bg-white" onClick={() => cartState.setQuantity(restaurant.id, item.id, quantity - 1)}><Minus className="h-4 w-4" /></button><span className="w-4 text-center text-sm font-bold">{quantity}</span><button aria-label={`Add one ${item.name}`} className="grid h-8 w-8 place-items-center rounded-full bg-ink text-white" onClick={() => cartState.addItem(restaurant.id, item.id)}><Plus className="h-4 w-4" /></button></div> : <Button size="sm" variant="secondary" onClick={() => cartState.addItem(restaurant.id, item.id)}><Plus className="h-4 w-4" />Add</Button>}</div></Card>; })}</div></section>)}</div>
    </section>}
    <CartSheet restaurant={restaurant} items={menuItems} />
  </main>;
}
