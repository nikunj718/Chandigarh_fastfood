...
type TrackingData = {
  ...
  deliveryQueue: "upcoming" | "current" | "next" | "waiting";
  preparation: { minutes: number; readyAt: string | null } | null;
  etaSeconds: number | null;
  estimatedDeliveryAt: string | null;
  route: RouteGeometry | null;
};

const labels: Record<string, string> = { pending_payment: "Awaiting payment", pending_approval: "Awaiting restaurant approval", confirmed: "Order confirmed", preparing: "Being prepared", prepared: "Prepared and awaiting rider", out_for_delivery: "On the way", delivered: "Delivered", cancelled: "Cancelled" };
...
export function TrackingExperience({ orderId }: { orderId: string }) {
  ...
  useEffect(() => {
    ...
    const refreshTracking = () => { void load().catch(() => setError("Live tracking briefly disconnected. Retrying…")); };
    const channel = supabase.channel(`delivery-${orderId}`)
      .on("postgres_changes", { event: "INSERT", schema: "public", table: "delivery_location_points", filter: `order_id=eq.${orderId}` }, refreshTracking)
      .on("postgres_changes", { event: "UPDATE", schema: "public", table: "orders", filter: `id=eq.${orderId}` }, refreshTracking)
      .subscribe();
  ...
  const steps: Array<{ status: string; Icon: LucideIcon; text: string }> = [
    ...
    { status: "prepared", Icon: CheckCircle2, text: "Prepared for rider pickup" },
    { status: "out_for_delivery", Icon: Truck, text: "Rider is on the way" },
    { status: "delivered", Icon: MapPin, text: "Delivered to your door" },
  ];
  return <main className="min-h-screen bg-cream"><header className="border-b border-orange-100 bg-white px-5 py-4"><div className="mx-auto flex max-w-6xl items-center justify-between"><Link href="/restaurants" className="font-bold">Fastfood Delivery</Link><span className="rounded-full bg-orange-100 px-3 py-1 text-sm font-bold text-saffron">Live delivery</span></div></header><section className="mx-auto grid max-w-6xl gap-6 px-5 py-8 lg:grid-cols-[360px_1fr]"><Card className="order-2 p-6 lg:order-1"><p className="text-xs font-bold uppercase tracking-[.16em] text-saffron">{tracking?.orderNumber ? `Order #${tracking.orderNumber}` : "Order tracking"}</p><h1 className="display-font mt-2 text-3xl">{tracking ? labels[tracking.status] ?? tracking.status : "Finding your order"}</h1>{error && <div className="mt-5"><StatusNote tone="error">{error}</StatusNote></div>}{tracking && <>{tracking.status === "out_for_delivery" && tracking.deliveryQueue === "next" && <div className="mt-5"><StatusNote>Your rider is dropping off another order nearby. You are next!</StatusNote></div>}{tracking.status === "out_for_delivery" && tracking.deliveryQueue === "waiting" && <div className="mt-5"><StatusNote>Your rider has other deliveries first. We’ll update you as you move up.</StatusNote></div>}<div className="mt-7 rounded-2xl bg-orange-50 p-4"><div className="flex items-center gap-3"><Clock3 className="h-5 w-5 text-saffron" /><div>{tracking.preparation ? <><p className="text-xs font-bold uppercase tracking-wide text-stone-500">Preparation time</p><p className="text-xl font-bold">{tracking.preparation.minutes} min</p>{tracking.preparation.readyAt && <p className="mt-1 text-xs text-stone-500">Ready around {new Date(tracking.preparation.readyAt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}</p>}</> : tracking.status === "prepared" ? <><p className="text-xs font-bold uppercase tracking-wide text-stone-500">Kitchen update</p><p className="text-xl font-bold">Ready for pickup</p></> : tracking.status === "out_for_delivery" && tracking.deliveryQueue === "current" ? <><p className="text-xs font-bold uppercase tracking-wide text-stone-500">Delivery ETA</p><p className="text-xl font-bold">{eta}</p>{tracking.estimatedDeliveryAt && <p className="mt-1 text-xs text-stone-500">Around {new Date(tracking.estimatedDeliveryAt).toLocaleTimeString([], { hour: "numeric", minute: "2-digit" })}</p>}</> : tracking.status === "out_for_delivery" ? <><p className="text-xs font-bold uppercase tracking-wide text-stone-500">Delivery queue</p><p className="text-xl font-bold">Waiting for rider</p></> : <><p className="text-xs font-bold uppercase tracking-wide text-stone-500">Order update</p><p className="text-xl font-bold">{labels[tracking.status]}</p></>}</div></div></div><ol className="mt-7 space-y-5 border-l border-orange-200 pl-5">{steps.map((step) => { const order = ["pending_payment", "pending_approval", "confirmed", "preparing", "prepared", "out_for_delivery", "delivered"].indexOf(tracking.status); const done = order >= ["confirmed", "preparing", "prepared", "out_for_delivery", "delivered"].indexOf(step.status); const ActiveIcon = step.Icon; return <li className={`relative flex gap-3 text-sm ${done ? "text-ink" : "text-stone-400"}`} key={step.status}><span className={`absolute -left-[31px] grid h-5 w-5 place-items-center rounded-full ${done ? "bg-saffron text-white" : "bg-orange-100"}`}><ActiveIcon className="h-3 w-3" /></span>{step.text}</li>; })}</ol><div className="mt-7 border-t border-orange-100 pt-5 text-sm"><p className="flex gap-2"><Store className="h-4 w-4 text-saffron" />{tracking.restaurant.name}</p><p className="mt-3 flex gap-2 text-stone-600"><MapPin className="h-4 w-4 text-moss" />{tracking.customerAddress.address_text}</p>{tracking.rider && <p className="mt-3 flex gap-2 text-stone-600"><Navigation className="h-4 w-4 text-saffron" />Rider updated {new Date(tracking.rider.recordedAt).toLocaleTimeString()}</p>}</div></>}</Card><div className="order-1 min-h-[460px] overflow-hidden rounded-3xl border border-orange-100 bg-orange-100 lg:order-2">{mapError ? <div className="grid h-full min-h-[460px] place-items-center p-8"><StatusNote tone="error">{mapError}</StatusNote></div> : <div ref={mapContainer} className="h-full min-h-[460px]" />}</div></section></main>;
}
