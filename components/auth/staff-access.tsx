  ...
  useEffect(() => {
    ...
    getSupabaseBrowserClient().auth.getUser().then(({ data }) => {
      ...
      if (data.user?.email && data.user.email_confirmed_at) {
        setMode("secure-account");
        if (awaitingConfirmation) setMessage("Email confirmed. Set a password to secure this restaurant owner account.");
      } else if (awaitingConfirmation) {
        setMode("secure-account");
        setMessage("We sent a confirmation link to your Gmail address. Confirm it, then return here to set a password.");
      }
    });
