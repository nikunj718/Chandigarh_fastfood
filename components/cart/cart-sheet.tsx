...
export function CartSheet({ restaurant, items }: { restaurant: Restaurant; items: MenuItem[] }) {
  ...
  async function checkout() {
    ...
    try {
      ...
      const response = await fetch("/api/checkout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ restaurantId: restaurant.id, addressId: selectedAddressId, paymentMethod, idempotencyKey: checkoutId.current, lines: validLines.map((line) => ({ itemId: line.itemId, quantity: line.quantity })) }) });
      const payload = await response.json();
