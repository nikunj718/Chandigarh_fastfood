...
export function CartSheet({ restaurant, items }: { restaurant: Restaurant; items: MenuItem[] }) {
  ...
  const [addressDraft, setAddressDraft] = useState<Suggestion | null>(null);
  ...
  async function saveAddress() {
    ...
    const response = await fetch("/api/addresses", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ label: "Delivery address", addressText: addressDraft.addressText, latitude: addressDraft.latitude, longitude: addressDraft.longitude, isDefault: addresses.length === 0 }) });
    const payload = await response.json();
    ...
    setSelectedAddressId(payload.id); setAddressQuery(""); setSuggestions([]); setAddressDraft(null);
  }
