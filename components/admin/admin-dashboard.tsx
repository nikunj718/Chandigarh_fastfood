...
export function AdminDashboard({ restaurantId }: { restaurantId: string }) {
  ...
  const [team, setTeam] = useState({ email: "", role: "manager" });
  ...
  return <main className="min-h-screen bg-cream">
    <header className="border-b border-orange-100 bg-white px-5 py-4"><div className="mx-auto flex max-w-6xl items-center justify-between gap-4"><Link href={`/restaurants/${restaurant.slug}`} className="font-bold">← {restaurant.name}</Link><div className="flex items-center gap-3">{data.membershipRole === "owner" && <Link href={`/admin/${restaurantId}/riders`} className="rounded-full bg-orange-100 px-3 py-1 text-sm font-bold text-saffron hover:bg-orange-200">Riders</Link>}<span className="rounded-full bg-ink px-3 py-1 text-sm font-bold text-white">Operations</span></div></div></header>
    <div className="mx-auto max-w-6xl px-5 py-8">
      ...
      <div className="grid gap-6 lg:grid-cols-[1.25fr_.75fr]">
        ...
        <Card className="p-6"><div className="flex items-center gap-2"><UsersRound className="h-5 w-5 text-saffron" /><h2 className="display-font text-2xl">Team access</h2></div><p className="mt-2 text-sm text-stone-600">Add confirmed manager accounts here. Rider invitations are managed from the dedicated Riders page.</p><form className="mt-5 space-y-3" onSubmit={(event) => void addMember(event)}><Input type="email" placeholder="Manager email" value={team.email} onChange={(event) => setTeam({ ...team, email: event.target.value })} /><Button size="sm" type="submit"><Plus className="h-4 w-4" />Add manager</Button></form><div className="mt-5 space-y-2">{data.members.filter((member) => member.role !== "rider").map((member) => <div className="flex items-center justify-between rounded-xl bg-orange-50 px-3 py-2 text-sm" key={member.user_id}><span>{member.profiles?.display_name || member.profiles?.email || member.user_id.slice(0, 8)}</span><span className="font-bold capitalize text-saffron">{member.role}</span></div>)}</div></Card>
      </div>
...
function MenuItemEditor({ restaurantId, item, categories, onSaved, onMessage }: { restaurantId: string; item: MenuItem; categories: AdminCategory[]; onSaved: () => Promise<void>; onMessage: (message: string) => void }) {
  ...
  async function deleteItem() {
    if (!window.confirm(`Delete ${item.name}? Existing orders will keep their saved dish details.`)) return;
    setSaving(true);
    try {
      const response = await fetch(`/api/admin/restaurants/${restaurantId}/menu/${item.id}`, { method: "DELETE" });
      const payload = await response.json();
      if (!response.ok) throw new Error(payload.error ?? "Menu item could not be deleted.");
      await onSaved();
      onMessage("Menu item deleted.");
    } catch (error) { onMessage(error instanceof Error ? error.message : "Menu item could not be deleted."); }
    finally { setSaving(false); }
  }
